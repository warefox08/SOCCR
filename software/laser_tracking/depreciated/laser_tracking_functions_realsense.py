...
def search_for_laser(dc, fov_h, res_h):
    ...
    #big = np.uint32(color_frame)
    #print("big:")
    #print(big)
    # cv2.imshow('big', big)
    
    enc = np.float32(color_frame)
    print("encoded:")
    print(enc)
    cv2.imshow('enc', enc)

    # Convert the BGR (opencv does BGR instead of RGB) color frame to a HSV frame
    hsv1 = cv2.cvtColor(color_frame, cv2.COLOR_BGR2HSV) 
    hsv_frame = cv2.cvtColor(enc, cv2.COLOR_BGR2HSV_FULL)

    print("HAV FRAMES 1 AND 2")
    cv2.imshow('HSV1', hsv1)
    print("hsv1 frame:")
    print(hsv1)
    print("hsv1 frame size:")
    print(np.size(hsv1))
    print("hsv1 frame shape:")
    print(hsv1.shape)
    print("dtype:")
    print(hsv1.dtype)
    print("HSV1 max")
    ...
    cv2.imshow('HSV2', hsv_frame)
    print("HSV2")
    print(hsv_frame)
    print("HSV2 size:")
    print(np.size(hsv_frame))
    print("hsv2 frame shape:")
    ...
    wtf = np.divide(hsv1, hsv_frame)
    print("dividing hsv1 and hsv2:")
    print(wtf)
    print("W matrix shape:")
    print(wtf.shape) 

    hsv_intm = np.rint(hsv_frame) 
   # cv2.imshow('intermediate', hsv_intm)
    hsv_rev = np.divide(hsv_intm, 2)
   # cv2.imshow('HSV reverted', hsv_rev)
    hsv_final = np.rint(hsv_rev)
   # print("HSV fin")
   # print(hsv_final)
   # cv2.imshow('final stage', hsv_final)
